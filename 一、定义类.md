...
在面向对象编程中，对象被分类成有等级的集合。对象被 *类* 编程小组，而不是像生物学一样用直接的专业术语分类。和人类从父母身上继承了一些特性一样，类也可以从它的 *父类(superclass)* 中继承特性。  
...
###基类提供最基本的功能  
就像所有活的有机体有着相同的"活"的特性一样，所有Objective-C的对象有着相同的一些最基本的功能。  

当一个Objective-C对象需要和另一个类的对象共同工作时，它们就需要提供一些基本的特性和方法。为此，Objective-C定义了一个所有类都继承于它的类————基类`NSObject`。当一个对象遇到另一个对象时，它们可以用来自`NSObject`声明的方法进行交互。  

当你定义一个类时，这个类最起码要继承自`NSObject`。总的来说，这个类要继承和它需要的功能最接近的Cocoa 或 Cocoa Touch的类。  

例如，你想给你的iOS app自定义一个按钮，但是Objective-C中提供的`UIButton`类并不能满足你的需求，你就应该创造一个继承自`UIButton`的新的类，而不是继承自`NSObject`。如果这个新的类是继承自`NSObject`的，你就要重复写那些`UIButton`已经提供了的功能。如果这个新的类是继承自`UIButton`的，那么这个类就拥有了`UIButton`的所有特性，只需要加上自己需要的功能就可以了。  

`UIButton`这个类本身是继承自`UIControl`的，`UIControl`描述了iOS系统中所有相同的用户界面操作。`UIControl`又是继承自`UIView`的，它定义了所有在屏幕上展示的对象的共有的特性和方法。`UIView`又继承自`UIResponder`，`UIResponder`能够响应用户的操作，例如点(tap)，手势(gesture)和摇晃(shake)。最后，在继承树的最底端，`UIResponder`继承自`NSObject`类。图1-3表示这些继承关系。  
...
这一连串的继承意味着一个新建的自定义的`UIButton`的子类不到你能继承`UIButton`声明的属性和方法，还能继承它每一个父类的属性和方法。这个子类的对象有着按钮的功能，可以在屏幕上展示，响应用户的操作，还能与所有Cocoa Touch的对象交互。  
...
##类的声明部分定义了它和外界的交互
面向对象编程有很多好处，其中一个就是当你使用一个类时你只需要知道怎样和这个类的实例交互就可以了。准确来说，一个对象的内部实现函数式是应该被隐藏起来的。  

例如，如果你在你的iOS app中使用`UIButton`，你不用担心按钮的像素是怎样显示的，你只需要知道你可以改变这个按钮的一些属性，例如按钮的标题和颜色，当你把这个按钮加到屏幕上时，它就会准确地按照你的要求显示出来。  

当你定义你自己的类时，你需要从定义这个类的公有属性和方法。哪些属性是可以被公开访问的？这些属性能被修改吗？其他对象怎么和你的对象交互？  

这些信息需要在你的类的声明部分中说明，它需要定义其他对象怎么和你的类的实例对象交互。公有方法和私有方法是分开定义的。在Objective-C中，类的声明部分和实现部分一般是放在不同的文件中的，因此你只需要把 *类的声明部分(interface)* 公有就行了。  
...
公有的属性和方法需要被定义在`@interface`这个声明部分里面。在这个例子中，没有定义和父类不一样的属性和方法，所以`SimpleClass`的对象所拥有的功能和`NSObject`的一模一样。  

###*属性(property)* 是对象的值的访问接口
...
在一些情况中，你需要声明一些不能被改变的属性。在现实世界中，一个人要改变他的名字是非常困难的。那么在我们的app中，我们可以把名字这个属性的可访问性设置为**read-only**，这样所有对这个属性的值的改变都需要一个 *中间对象(intermediary object)* 来控制这个值的改变能否生效。  

Objective-C中可以声明 *属性的属性property attributes*，用来指示这个属性可读性和其他属性。在我们的app中，我们可以把`Person`类的interface部分修改成这样：
```
@interface Person : NSObject
...
###方法的声明
目前为止，我们的例子描述了一个典型的对象，或者换句话说，一个简单的用来封装数据的对象。在`Person`类中，不需要方法来操作它的属性也是可以的。但是大多数的类，出了声明它的属性，还需要声明它的方法。  

用Objective-C写的程序都是建立在巨大的各种对象的联系网之上的，因此对象通过发送信息来和其他对象交互是非常重要的。在Objective-C中，一个对象通过调用另一个函数的方法来传递信息。  

Objective-C的方法（函数）和C语言以及其他语言的方法（函数）在概念上时相似的，但是定义的语法有点不同。在C语言中，方法的定义是这样的：
```
void SomeFunction();
```
...
-(void)someMethod;
```
这个函数没有参数。C语言的关键字`void`在Objective-C中被放在了括号里面，而且放在方法声明的开始部分，用来表示这个方法在结束时不返回值。  
...
就像C语言一样，Objective-C的方法声明也需要一个分号`;`结尾。  
...
如果你需要传入多个参数，语法和C语言就更加不一样了。C语言的多个参数都放在一个括号里面，用括号隔开；而在Objective-C中，两个参数的函数的声明是这样的：
```
- (void)someMethodWithFirstValue:(SomeType)value1 secondValue:(AnotherType)value2;
```
在这个例子中，`value1`和`value2`是函数在实现时的内部变量名，它们和你调用这个函数时传入的参数一一对应。  

在一些编程语言中，允许在函数的定义中加入 *命名参数(named arguements)* ；但是在Objective-C中这是不允许的。调用函数时参数的顺序必须和函数在声明的时候参数的顺序一致，事实上`secondValue:`是函数名字的一部分：
```
someMethodWithFirstValue:secondValue:
```
这种写法使得Objective-C成为一门可读性很强的语言，因为函数的名字中会指出下一个传入的参数的名字，详情请参考**You Can Pass Objects for Method Parameters**这一节。  

**注意**：`value1`和`value2`这两个变量名并不是严格要求出现在函数名里的，也不要求函数名里出现的指示和参数名完全一致，只要能起到一个标志指示的作用就行了。在写函数的实现的时候，要求函数名、参数类型、返回类型和声明时完全一致。  
...
**历史小知识：**如果你想知道为什么你遇到的类中为什么很多都有`NS`的前缀，那就要从Cocoa 和 Cocoa Touch的历史说起了。Cocoa是NeXT公司(乔布斯在第一次离开苹果公司后开的公司)为了在NeXTStep操作系统上写程序而开发的框架。乔布斯回归苹果后，苹果公司在1996年收购了NeXT公司，并且把大部分NeXTStep操作系统合并到了OS X操作系统，其中就包括了很多写好的类名。而Cocoa Touch则是作为Cocoa的替代物被引入到iOS操作系统的；尽管两个框架间有很多类都是不一样的，但仍有一些类在Cocoa 和 Cocoa Touch都能使用。像`NS`和`UI`(代表iOS的用户界面User Interface)这种两个字母的前缀就被苹果保留了起来。  

相比较之下，方法名和属性名只需要在类里面保持唯一就可以了。尽管在C语言中函数名必须在一个工程中唯一，但是在Objective-C中我们鼓励在不同的类中使用相同的函数名。在同一个类的声明中，一个函数我们只能声明一次，但是如果你想重载一个继承而来的函数的话，你必须使用和父类中与该函数完全一致的函数名。  

像函数一样，一个对象的属性和 *实参(instace variable)*只需要在同一个类中保持唯一就可以了（详情请见**Most Properties Are Backed by Instance Variables**)这一节。但是，如果你想使用全局变量，这些变量名就必须在app或者工程中是唯一的。  
...
一开始，一个类的声明部分通常放在一个叫 *头文件(header file)* 的文件中，这个文件一般以`.h`作为扩展名。而你需要在以`.m`为拓展名的源代码文件里为你的类写实现的代码。  
...
@end
```
这个例子使用了`NSLog（）`这个函数来在控制台打印一些信息。这个函数和C语言中的`printf()`函数是一样的。`NSLog（）`函数可以传入一些参数，但第一个参数必须是Objective-C的字符串。  

就像C语言的函数一样，Objective-C的函数用花括号`{}`把函数的实现代码包起来。而且函数的名字、参数类型和返回类型都必须和声明时一模一样。  

Objective-C和C语言一样，对函数名是敏感的，所以像这个函数：
```
- (void)sayhello {
...
总的来说，函数名的第一个字母应该是小写的。Objective-C和C语言的命名传统不一样，Objective-C会使用更多描述性的语言来为这个函数命名。如果一个函数名中有很多个单词，就应该使用 *驼峰命名法(camel case)* 来增强这个函数名的可读性。  
...
在下一章**Working with Objects**中你会看到更多函数实现的例子。  
